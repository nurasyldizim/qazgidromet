        ...
        try {
            ...
            if (action.equals("Save")) {
                ...
                try {
                    ...
                    String isDataAdded = pnzDataDao.addPnzData(pnz, pnzDateTime, bsh, ds, sr, ou, do_, oa, ozon, serovodorod, fenol, fv, hlor, hv, ammiak, skIs, formaldigid, nsm, hromSh, sumU);
                    if (isDataAdded == null) {
                        HttpSession session = request.getSession();
                        session.setAttribute("errMessage", "Данные не добавлены. На этот день и время уже есть данные");
                        ...
                        HttpSession session = request.getSession();
                        session.setAttribute("errMessage", "Данные успешно добавлены.");
                        response.sendRedirect("jsp/pnzdata.jsp?pnzId=" + pnzId + "&pnzName=" + URLEncoder.encode(pnzName, "UTF-8") + "&month=" + month + "&cityId=" + cityId + "&year=" + year + "&cityName=" + URLEncoder.encode(cityName, "UTF-8"));
                        response.sendRedirect(action);
                    }
                } catch (Exception e) {
                ...
                try {
                    PnzDataDao pnzDataDao = new PnzDataDao();
                    String isDataUpdated = pnzDataDao.updatePnzData(Integer.parseInt(pnzDataId), pnz, pnzDateTime, bsh, ds, sr, ou, do_, oa, ozon, serovodorod, fenol, fv, hlor, hv, ammiak, skIs, formaldigid, nsm, hromSh, sumU);
                    if (isDataUpdated == null) {
                        HttpSession session = request.getSession();
                        session.setAttribute("errMessage", "Данные не измененеы. Проверте данные еще раз");
                        response.sendRedirect("jsp/pnzdata.jsp?pnzId=" + pnzId + "&pnzName=" + URLEncoder.encode(pnzName, "UTF-8") + "&month=" + month + "&cityId=" + cityId + "&year=" + year + "&cityName=" + URLEncoder.encode(cityName, "UTF-8"));
                    } else {
                        HttpSession session = request.getSession();
                        session.setAttribute("errMessage", "Данные успешно изменены.");
                        response.sendRedirect("jsp/pnzdata.jsp?pnzId=" + pnzId + "&pnzName=" + URLEncoder.encode(pnzName, "UTF-8") + "&month=" + month + "&cityId=" + cityId + "&year=" + year + "&cityName=" + URLEncoder.encode(cityName, "UTF-8"));
                    }
                } catch (Exception e) {
